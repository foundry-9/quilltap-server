...
export function DescriptionSourceStep({
  ...
  return (
    <div className="space-y-6">
        ...
        {sourceOptions.map((option) => (
          ...
          >
            ...
            <div>
              ...
              <div className="text-sm qt-text-secondary">{option.description}</div>
            </div>
          </label>
        ))}
      </div>
      ...
      {/* Upload Section */}
      {source === 'upload' && (
        <div className="p-4 rounded-lg border qt-border-default qt-bg-muted/20 space-y-4">
          <h4 className="font-medium text-foreground">Upload Image</h4>

          {uploadedImageUrl ? (
            <div className="space-y-3">
              <div className="relative w-32 h-32 rounded-lg overflow-hidden border qt-border-default">
                { }
                <img
              ...
              <button
                ...
                className="text-sm qt-text-destructive hover:underline"
              >
                Remove image
              </button>
            </div>
            ...
            <div>
              ...
              >
                {uploading ? (
                  ...
                  <>
                    <svg className="w-8 h-8 qt-text-secondary mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                    <span className="text-sm qt-text-secondary">Click to upload image</span>
                    <span className="text-xs qt-text-secondary mt-1">JPEG, PNG, GIF, WebP</span>
                  </>
                )}
              </label>
            </div>
      ...
      {source === 'document' && (
        <div className="p-4 rounded-lg border qt-border-default qt-bg-muted/20 space-y-4">
          ...
          {uploadedDocumentName ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 rounded-lg border qt-border-default bg-background">
                <svg className="w-8 h-8 qt-text-secondary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-foreground truncate">{uploadedDocumentName}</div>
                  <div className="text-sm qt-text-secondary">Document uploaded</div>
                </div>
              </div>
              <button
                type="button"
              ...
              >
                {uploadingDocument ? (
                  ...
                  <>
                    ...
                    <span className="text-sm qt-text-secondary">Click to upload document</span>
                    <span className="text-xs qt-text-secondary mt-1">Text, Markdown, or PDF</span>
                  </>
                )}
              </label>
            </div>
          ...
          {documentUploadError && (
            <p className="text-sm qt-text-destructive">{documentUploadError}</p>
          )}
        </div>
      )}
      ...
      {needsVisionProfile && (source === 'upload' || source === 'gallery') && (
        <div className="p-4 rounded-lg border qt-border-warning/50 qt-bg-warning/10 space-y-3">
          <div className="flex items-start gap-2">
            <svg className="w-5 h-5 qt-text-warning mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
