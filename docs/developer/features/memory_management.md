...
### 6.1 `searchMemoriesSemantic` full-table hydration — PRIME SUSPECT, UNFIXED

`lib/memory/memory-service.ts:515`
...
Every single semantic search — and there's at least one per chat turn, often more if the tool search path fires — loads **every memory for the character** from SQLite, runs Zod validation on the entire array, and builds a Map, **just to hydrate the top-K vector-search results** (typically 60 rows out of tens of thousands).

On 32k memories this is:

- one encrypted SELECT with 32k rows
- 32k Zod `MemorySchema.parse` calls
- 32k `Map.set` calls
- a dead-weight allocation of everything that isn't in the top-K

This call is **synchronous from the event loop's perspective** — there is no `yield` between row validations. Each chat turn pays this cost. The Memory Gate already has the right pattern on line 161 of memory-gate.ts:
...
The retrieval path should be rewritten the same way. This is almost certainly the largest remaining single contributor to the stalls the user is observing in the Salon during chats, and nothing in the 2026-04-20 commits touched it.
...
1. **Fix `searchMemoriesSemantic` to hydrate only matched ids** (§6.1). Mirror the gate's pattern: `findByIds(vectorResults.map(r => r.id))`. Expected win is per-turn and unconditional.
...
3. **Paginate the embedding-repair handlers** (§6.9) the same way housekeeping was paginated in `734a9f31`. Low-impact per incident but user-initiated, so visible when it fires.
4. **Add a UI for `memoryExtractionLimits`** so the user can actually throttle extraction on a runaway chat without editing settings.json. API-only today.
5. **Search-hit counter as a retrieval signal.** Tracked in `project_search_hit_counter.md`. Bumps a per-memory counter when tool-search retrieves it; feeds back into the blended protection score as a softer proxy alongside `lastAccessedAt`.
6. **Rank-normalize LLM importance.** Tracked in `project_importance_rank_normalize.md`. Optional sharpening layer; only worth trying if the cheap-LLM importance output is monotonic enough that percentile normalization helps.
