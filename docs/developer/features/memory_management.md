...
5. **Search-hit counter as a retrieval signal.** Tracked in `project_search_hit_counter.md`. Bumps a per-memory counter when tool-search retrieves it; feeds back into the blended protection score as a softer proxy than `lastAccessedAt`.
