...
Every conversation you keep, Quilltap keeps in full — every word, forever, or until you say otherwise. But a conversation also gathers a certain amount of behind-the-scenes paraphernalia as it goes: compression caches, pre-rendered pages, and the models' own scratch-work and thinking traces. In a busy establishment, the quiet rooms end up storing rather a lot of this — none of it part of the story, all of it faithfully rebuilt on demand.

The mathematical fingerprints (embeddings) that let semantic search find a conversation by meaning rather than by exact words are a separate matter entirely: they cost very little to keep, so Quilltap keeps every one of them warm, whether the chat was spoken in yesterday or a year ago. Semantic search over your whole conversation history therefore works uniformly, with nothing to re-index and no waiting for an old room to warm back up.
...
Never touched, at any age: the messages themselves, attachments, memories, conversation summaries, conversation embeddings, and anything you deliberately saved.

## Search stays warm, always

Unlike the caches above, a conversation's semantic-search fingerprints are never tidied away for being quiet — **keyword search** and **semantic search** alike work on an old, untouched conversation exactly as they do on one from this morning. If a conversation is ever missing its embeddings (an interrupted render, an embedding-provider outage), Quilltap notices and quietly re-indexes it in the background the next time you open it, or you can force the matter for any conversation by clicking its **Scriptorium status badge** on the chat's card in the Salon list, which re-renders and re-embeds the whole affair.
