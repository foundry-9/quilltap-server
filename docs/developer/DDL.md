...
CREATE TABLE "chats" (
  ...
  "moderationRefusalCount" INTEGER NOT NULL DEFAULT 0,  -- The Concierge's refusal ledger: stated moderation refusals (typed-error, provider-code, finish-reason, message-pattern; never 'inferred') since the chat was last set to Monitored. Incremented atomically (SET v = v + 1) only by ChatsRepository.incrementModerationRefusalCount, called only by recordModerationRefusal (lib/services/dangerous-content/refusal-ledger.ts); reset to 0 only by applyConciergeFlip's 'monitored' case. At autoSwitchAfterRefusals on a Monitored AUTO_ROUTE chat the Concierge switches it to Flagged. **Deliberately absent from ChatMetadataSchema**, like transcriptVersion, so a whole-row update() cannot rewind it; read it with ChatsRepository.getModerationRefusalLedger. Not exported in .qtap bundles. Added by add-chat-refusal-ledger-v1.
