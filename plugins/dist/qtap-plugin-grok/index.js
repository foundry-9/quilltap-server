...
var FileTransport = class {
  ...
  async initializeDirectory() {
    try {
      ...
      const combinedLogPath = (0, import_path33.join)(this.logDir, "combined.log");
      const errorLogPath = (0, import_path33.join)(this.logDir, "error.log");
      try {
  ...
  async writeToFile(filename, content) {
    try {
      const filePath = (0, import_path33.join)(this.logDir, filename);
...
var GrokImageProvider = class {
  ...
  async validateApiKey(apiKey) {
    try {
      ...
      logger.error("Grok API key validation failed for image generation", { context: "GrokImageProvider.validateApiKey" }, error instanceof Error ? error : void 0);
      return false;
    }
  }
...
var plugin = {
  ...
  getAvailableModels: async (apiKey, baseUrl) => {
    ...
    try {
      ...
      logger.error("Failed to fetch Grok models", { context: "plugin.getAvailableModels" }, error instanceof Error ? error : void 0);
      return [];
    }
  },
  /**
  ...
  validateApiKey: async (apiKey, baseUrl) => {
    ...
    try {
      ...
      logger.error("Error validating Grok API key", { context: "plugin.validateApiKey" }, error instanceof Error ? error : void 0);
      return false;
    }
  },
  /**
