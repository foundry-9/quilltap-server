...
- **The outcome table.** An ordered cascade, checked top to bottom, first row whose every condition holds wins. Conditions are chips — a subject (the value, the raw roll, a parameter, or a key on the invoking character's [fact sheet](character-editing.md)), a comparator, and an operand — joined by AND. The final row is pinned: it reads *otherwise*, it cannot be moved or deleted, and so the format's mandatory catch-all simply cannot be violated. Metadata subjects take any key you care to type (the sheet is the character's vocabulary, not the file's) and offer all six comparators, with the gentle reminder that an ordering test against a key that holds a string declines the row at run time — fail-soft, never an error.
- **Messages.** Each outcome's message takes placeholders — `{{value}}`, `{{roll}}`, `{{dice}}`, `{{params.x}}`, `{{metadata.key}}` — via the *Insert value* menu or your own typing. A typo'd placeholder gets a warning, not a veto: the runtime leaves unknown placeholders as written, so the Workbench merely raises an eyebrow. `{{metadata.…}}` is never called unknown — those keys live on characters, not in the file.
...
- **The audit.** *Deal a thousand hands* runs ten thousand draws with the current parameters and fact sheet and charts what share each outcome took. A row that never fired is flagged — with the honest caveat that reachability depends on the parameters and the sheet you supplied: a metadata-gated row showing nought under an empty sheet is working as designed, not broken.
