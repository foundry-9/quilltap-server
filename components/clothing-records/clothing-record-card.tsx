...
interface ClothingRecordCardProps {
  record: ClothingRecord
  onEdit: (record: ClothingRecord) => void
  onDelete: (id: string) => void
  isDeleting?: boolean
}

export function ClothingRecordCard({
  record,
  onEdit,
  onDelete,
  isDeleting,
}: ClothingRecordCardProps) {
          ...
          <div className="flex items-center gap-2 mt-1 qt-text-small">
            ...
            {record.usageContext && (
              <>
                ...
                <span className="italic">{record.usageContext}</span>
              </>
            )}
