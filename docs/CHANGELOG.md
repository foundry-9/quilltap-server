...
- Unit tests: `store-file.test.ts`, `read-file.test.ts` (mount-index + file-storage suites green — 221 tests). Follow-ups (not in this commit): document/redirect the generic `/api/v1/files` CRUD boundary, switch in-repo front-end callers to the item route, and wire the CLI verbs (publish-gated).
