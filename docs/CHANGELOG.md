...
- Known follow-up (not fixed here): `writeCharacterAvatarToVault` and `writeLanternBackgroundToMountStore` consume `linkBlobContent`'s `blobId` on their project-less vault paths, so they remain latent-broken in the child and need host-RPC (the pattern `FileStorageManager.uploadFile` already uses). My change leaves them failing loudly rather than corrupting state.
