...
CREATE TABLE "chat_messages" (
  ...
  "routeTrail" TEXT DEFAULT NULL              -- Message route trail: JSON [{ profileId, profileName, provider, modelName, via, outcome, trigger?, evidence?, detail? }] — every connection profile tried for this turn, in the order tried. `via` says how the profile came to be asked ('primary' | 'retry' | 'concierge' | 'understudy' | 'tier-pick'); `outcome` is 'answered' | 'failed' (fell over on its own) | 'refused' (declined on content grounds); `trigger` is the fallback engine's failure class ('auth' | 'rate-limit' | 'network' | 'model-missing' | 'provider-error' | 'empty-response' | 'moderation-refusal'), absent on the entry that answered; `evidence` says how a refusal was established ('finish-reason' — the provider said so — or 'inferred' from an empty body on a Concierge-flagged turn); `detail` is a short reason capped at 200 chars, never the full error body. The last entry always agrees with this row's provider/modelName, which stay authoritative for who answered. NULL on every message whose turn had no failure (nearly all of them — a one-entry trail would say nothing provider/modelName do not) and on every non-ASSISTANT row. Added by add-route-trail-message-column-v1.
);
