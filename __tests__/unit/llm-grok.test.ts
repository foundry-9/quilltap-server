 ...
 * Tests lib/llm/grok.ts
 * Grok is an OpenAI-compatible API from x.ai
 */
...
describe('GrokProvider', () => {
  ...
  describe('constructor', () => {
    it('should create provider with hardcoded Grok base URL', () => {
  ...
  describe('sendMessage', () => {
    ...
    it('should send a message and return formatted response', async () => {
      ...
      const result = await provider.sendMessage(mockParams, 'xai-api-key')
    ...
    it('should throw error without API key', async () => {
      await expect(provider.sendMessage(mockParams, '')).rejects.toThrow(
        'Grok provider requires an API key'
      )
    ...
    it('should use default values for optional parameters', async () => {
      ...
      await provider.sendMessage(minimalParams, 'xai-api-key')
    ...
    it('should handle custom temperature, maxTokens, topP, and stop sequences', async () => {
      ...
      await provider.sendMessage(customParams, 'xai-api-key')
    ...
    it('should propagate errors from the API', async () => {
      const apiError = new Error('Grok API Error')
      mockOpenAIInstance.chat.completions.create.mockRejectedValue(apiError)

      await expect(provider.sendMessage(mockParams, 'xai-api-key')).rejects.toThrow(
        'Grok API Error'
      )
    })
  })
  ...
  describe('streamMessage', () => {
    ...
    it('should throw error without API key when streaming', async () => {
      ...
      await expect(generator.next()).rejects.toThrow('Grok provider requires an API key')
    ...
    it('should use custom parameters for streaming', async () => {
      ...
      const chunks: any[] = []
      for await (const chunk of provider.streamMessage(customParams, 'xai-api-key')) {
        chunks.push(chunk)
      }
  ...
  describe('validateApiKey', () => {
    it('should return true when models endpoint is accessible', async () => {
      ...
      const result = await provider.validateApiKey('xai-api-key')
  ...
  describe('getAvailableModels', () => {
    it('should return sorted list of Grok models', async () => {
      const mockModels = {
        data: [
      ...
      const result = await provider.getAvailableModels('xai-api-key')

      expect(result).toEqual(['grok-1', 'grok-2', 'grok-beta'])
    })
    ...
    it('should return empty array without API key', async () => {
      ...
      expect(result).toEqual([])
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Grok provider requires an API key to fetch models'
      )
      // Should not even attempt to create client
      expect(mockOpenAI).not.toHaveBeenCalled()
    })
    ...
    it('should handle Grok-specific model names', async () => {
      const mockModels = {
        data: [
          { id: 'grok-beta', object: 'model' },
          { id: 'grok-1', object: 'model' },
      ...
      expect(result).toContain('grok-beta')
      expect(result).toContain('grok-vision-beta')
      expect(result[0]).toBe('grok-1') // Verifies sorting
    })
  })
})
