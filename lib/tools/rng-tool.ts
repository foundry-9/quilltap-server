 ...
 */
export const anthropicRngToolDefinition = {
  name: 'rng',
  description:
    'Generate random results for games and roleplay. Use for dice rolls, coin flips, or randomly selecting a participant. Results become permanent messages in the chat visible to all characters. Examples: roll a d20, flip a coin, spin the bottle to pick who goes next.',
  input_schema: {
 ...
 */
export function getOpenAIRngTool() {
  return rngToolDefinition;
}

/**
 * Helper to get tool definition in Anthropic format
 */
export function getAnthropicRngTool() {
  return anthropicRngToolDefinition;
}

/**
 * Helper to get Google/Gemini format tool definition
 */
export function getGoogleRngTool() {
  return {
    name: anthropicRngToolDefinition.name,
    description: anthropicRngToolDefinition.description,
    parameters: anthropicRngToolDefinition.input_schema,
  };
}
