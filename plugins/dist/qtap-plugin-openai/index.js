        ...
        for await (const sse of messages) {
          ...
          if (sse.event === null || !sse.event.startsWith("thread.")) {
            ...
            try {
              ...
              logger6.error(`Could not parse message into JSON:`);
              logger6.error(`From chunk:`);
...
}, _OpenAI_sanitizeLogger = function _OpenAI_sanitizeLogger2(logger6) {
  if (__classPrivateFieldGet(_a3, _a3, "f", _OpenAI_sanitizedLoggers).has(logger6))
    return logger6;
  const sanitized = new globalThis.Proxy(/* @__PURE__ */ Object.create(null), {
    get: (_facade, property) => {
      const value = globalThis.Reflect.get(logger6, property, logger6);
      if (typeof value !== "function")
        return value;
      if (typeof property !== "string" || !["debug", "info", "warn", "error", "trace", "log"].includes(property)) {
        return value.bind(logger6);
      }
      return (...args) => {
        const seen = /* @__PURE__ */ new WeakMap();
        return globalThis.Reflect.apply(value, logger6, args.map((entry) => __classPrivateFieldGet(this, _OpenAI_instances, "m", _OpenAI_sanitizeLogValue).call(this, entry, seen)));
      };
    },
    set: (_facade, property, value) => globalThis.Reflect.set(logger6, property, value, logger6)
  });
...
var OpenAIProvider = class {
  ...
  hasRefusal(response) {
    for (const item of response.output) {
      ...
      if (item.type === "message" && Array.isArray(item.content) && item.content.some((part) => part?.type === "refusal")) {
        return true;
      }
    }
    return false;
  ...
  buildLLMResponse(response, attachmentResults) {
    ...
    return {
      ...
      usage: {
        ...
        // display. (OpenAI folds cached_tokens into input_tokens.)
  ...
  async *streamMessage(params, apiKey) {
    ...
    if (finalResponse) {
      ...
      yield {
        ...
        usage: {
          // Cache-read tokens excluded from prompt/total (see buildLLMResponse).
...
var GPT_IMAGE_STANDARD_SIZES = ["auto", "1024x1024", "1536x1024", "1024x1536"];
var GPT_IMAGE_WIDE_SIZES = [
  "auto",
  "1024x1024",
  "1536x1024",
  "1024x1536",
...
var GPT_IMAGE_25_QUALITIES = ["auto", "low", "medium", "high", "xhigh", "max"];
...
var OPENAI_IMAGE_MODEL_IDS = OPENAI_IMAGE_MODELS.map((m) => m.id);
function findImageModel(model) {
  if (!model) {
    return void 0;
  }
...
function checkArbitrarySize(size) {
...
var MODERATION_CODES = /* @__PURE__ */ new Set(["moderation_blocked", "content_policy_violation"]);
function toOpenAIImageModerationError(error) {
  if (!error || typeof error !== "object") return error;
  const err = error;
  const code = typeof err.code === "string" ? err.code : typeof err.error?.code === "string" ? err.error.code : void 0;
  ...
  if (code && MODERATION_CODES.has(code) || message.toLowerCase().includes("safety system")) {
    return new ModerationRejectionError(
      ...
      typeof err.status === "number" ? err.status : void 0,
      code,
      "qtap-plugin-openai"
    );
  }
...
var OUTPUT_FORMATS = ["png", "jpeg", "webp"];
var BACKGROUNDS = ["auto", "transparent", "opaque"];
var MODERATIONS = ["auto", "low"];
var TRANSPARENCY_CAPABLE_FORMATS = ["png", "webp"];
var COMPRESSIBLE_FORMATS = ["jpeg", "webp"];
function readEnum(bag, key, allowed) {
  const raw = bag?.[key];
  ...
  logger2.warn("Ignoring unsupported OpenAI image parameter value", {
...
function readIntInRange(bag, key, min, max) {
  const raw = bag?.[key];
  if (raw === void 0 || raw === null || raw === "") {
    return void 0;
  }
  ...
  if (!Number.isFinite(value) || !Number.isInteger(value) || value < min || value > max) {
    logger2.warn("Ignoring out-of-range OpenAI image parameter", {
...
var OpenAIImageProvider = class {
  constructor() {
    ...
    this.supportedModels = [...OPENAI_IMAGE_MODEL_IDS];
  }
  ...
  validateAndNormalizeSize(size, model) {
    ...
    if (caps.arbitrarySizes) {
      ...
      if (check.ok) {
        ...
        if (parsed.width * parsed.height > ARBITRARY_SIZE_RULES.experimentalAbovePixels) {
          logger2.debug("Requesting an experimental OpenAI image resolution", {
      ...
      logger2.warn("Falling back to 1024x1024: unsupported OpenAI image size", {
        context: "OpenAIImageProvider.validateAndNormalizeSize",
        model,
        size,
        reason: check.reason
    ...
    logger2.warn("Falling back to 1024x1024: size not supported by this OpenAI model", {
      context: "OpenAIImageProvider.validateAndNormalizeSize",
      model,
      size,
   ...
   */
  resolveQuality(quality, model) {
    const caps = findImageModel(model);
    const isGptImage = this.isGptImageModel(model);
    if (!quality) {
      return isGptImage ? void 0 : "standard";
    }
    if (!caps) {
      return quality;
    }
    ...
    logger2.warn("Ignoring quality tier unsupported by this OpenAI model", {
  ...
  async generateImage(params, apiKey) {
    ...
    if (n !== requestedN) {
      logger2.warn("Capping image count to the model maximum", {
    ...
    if (isGptImage) {
      ...
      if (background !== void 0) {
        if (background === "transparent" && outputFormat !== void 0 && !TRANSPARENCY_CAPABLE_FORMATS.includes(outputFormat)) {
          logger2.warn("Forcing PNG output: a transparent background needs png or webp", {
            context: "OpenAIImageProvider.generateImage",
            model: modelName,
      ...
      if (outputCompression !== void 0) {
        if (outputFormat !== void 0 && COMPRESSIBLE_FORMATS.includes(outputFormat)) {
          requestParams.output_compression = outputCompression;
        } else {
          logger2.debug("Dropping output_compression: it applies only to jpeg and webp", {
            context: "OpenAIImageProvider.generateImage",
            model: modelName,
      ...
      if (moderation !== void 0) {
        requestParams.moderation = moderation;
      }
    }
    logger2.debug("Calling OpenAI Images API", {
      context: "OpenAIImageProvider.generateImage",
      model: modelName,
    ...
    try {
      ...
      if (mapped !== error) {
        logger2.info("OpenAI Images API refused the request on content grounds", {
          context: "OpenAIImageProvider.generateImage",
          model: modelName,
          providerReason: mapped.providerReason
        });
      }
      throw mapped;
    }
...
// image-options-schema.ts
var MODEL_DEFAULT = { value: "", label: "(model default)" };
var QUALITY_LABELS = {
...
function sizeDescription(size) {
  ...
  return parsed.width * parsed.height > ARBITRARY_SIZE_RULES.experimentalAbovePixels ? "Experimental resolution \u2014 slower, and the model may decline it." : void 0;
}
function qualityField(caps) {
  return {
    key: "quality",
    label: "Quality",
    type: "enum",
    default: "",
    helpText: caps.qualities.includes("max") ? "How much effort the model spends on the image. Extra High and Max are GPT Image 2.5\u2019s premium tiers \u2014 sharper detail, at a higher price and a longer wait." : "How much effort the model spends on the image. Higher tiers cost more and take longer.",
    enumValues: [
      MODEL_DEFAULT,
...
var STYLE_FIELD = {
  ...
  type: "enum",
  default: "",
  helpText: "DALL\xB7E 3 only. Vivid leans dramatic and hyper-real; Natural is more restrained.",
  enumValues: [
    MODEL_DEFAULT,
...
function getOpenAIImageOptionsSchema(modelName) {
  const caps = findImageModel(modelName) ?? OPENAI_IMAGE_MODELS[0];
  const fields = [qualityField(caps), sizeField(caps)];
  ...
  if (caps.gptImage) {
    groups.push({
      ...
      helpText: "Parameters the GPT Image families accept. DALL\xB7E models ignore this section because it is not offered for them.",
      fields: [
        BACKGROUND_FIELD,
        OUTPUT_FORMAT_FIELD,
...
var plugin = {
   ...
   */
  getImageProviderOptionsSchema: (context) => getOpenAIImageOptionsSchema(context?.modelName),
  /**
