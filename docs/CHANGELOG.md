...
- Follow-up (not in this commit): switch the in-repo Scriptorium front-end callers from `/blobs` to the `…/files/[...path]` item route (functional consolidation already complete since `/blobs` POST now delegates to `storeMountFile` — this is clean-cut polish).
