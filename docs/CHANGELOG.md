...
- fix: Sidebar navigation links unclickable on chat page — Next.js `<Link>` components in the left sidebar silently failed to navigate when on the salon/[id] page, likely due to `startTransition` being perpetually deferred by the chat page's heavy concurrent rendering; replaced `<Link>` with `<a>` + `router.push()` in collapsed-nav, sidebar-header, sidebar-footer, and sidebar-item components, matching the pattern already proven working in profile-menu
