...
}, _OpenAI_sanitizeLogger = function _OpenAI_sanitizeLogger2(logger5) {
  if (__classPrivateFieldGet(_a3, _a3, "f", _OpenAI_sanitizedLoggers).has(logger5))
    return logger5;
  const sanitized = new globalThis.Proxy(/* @__PURE__ */ Object.create(null), {
    get: (_facade, property) => {
      const value = globalThis.Reflect.get(logger5, property, logger5);
      if (typeof value !== "function")
        return value;
      if (typeof property !== "string" || !["debug", "info", "warn", "error", "trace", "log"].includes(property)) {
        return value.bind(logger5);
      }
      return (...args) => {
        const seen = /* @__PURE__ */ new WeakMap();
        return globalThis.Reflect.apply(value, logger5, args.map((entry) => __classPrivateFieldGet(this, _OpenAI_instances, "m", _OpenAI_sanitizeLogValue).call(this, entry, seen)));
      };
    },
    set: (_facade, property, value) => globalThis.Reflect.set(logger5, property, value, logger5)
  });
...
// provider.ts
var NANOGPT_SUPPORTED_IMAGE_MIME_TYPES = [
  "image/jpeg",
  "image/png",
  "image/gif",
  "image/webp"
];
var NANOGPT_PROFILE_PARAM_ALLOWLIST = [
  "frequency_penalty",
  "presence_penalty",
  "logprobs",
  "top_logprobs",
...
var NanoGPTProvider = class extends OpenAICompatibleProvider {
  constructor(config2) {
    super({
      baseUrl: config2?.baseUrl ?? NANOGPT_BASE_URL,
      providerName: config2?.providerName ?? "NanoGPT",
      requireApiKey: config2?.requireApiKey ?? true,
      attachmentErrorMessage: config2?.attachmentErrorMessage ?? "NanoGPT forwards images only for image MIME types NanoGPT accepts."
    });
    this.profileParamAllowlist = NANOGPT_PROFILE_PARAM_ALLOWLIST;
  }
...
var NanoGPTImageProvider = class {
  constructor() {
    ...
    this.supportedModels = STATIC_IMAGE_MODEL_IDS;
    this.baseUrl = NANOGPT_BASE_URL;
  }
...
var attachmentSupport = {
  ...
  notes: "NanoGPT routes to hundreds of upstream models, so the plugin forwards images for any model and lets the host decide: attachments only reach a profile whose supportsImageUpload flag is set, and the describe-fallback substitutes text otherwise."
};
var messageFormat = {
