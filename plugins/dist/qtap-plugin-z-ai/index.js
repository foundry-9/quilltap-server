...
}, _OpenAI_sanitizeLogger = function _OpenAI_sanitizeLogger2(logger4) {
  if (__classPrivateFieldGet(_a3, _a3, "f", _OpenAI_sanitizedLoggers).has(logger4))
    return logger4;
  const sanitized = new globalThis.Proxy(/* @__PURE__ */ Object.create(null), {
    get: (_facade, property) => {
      const value = globalThis.Reflect.get(logger4, property, logger4);
      if (typeof value !== "function")
        return value;
      if (typeof property !== "string" || !["debug", "info", "warn", "error", "trace", "log"].includes(property)) {
        return value.bind(logger4);
      }
      return (...args) => {
        const seen = /* @__PURE__ */ new WeakMap();
        return globalThis.Reflect.apply(value, logger4, args.map((entry) => __classPrivateFieldGet(this, _OpenAI_instances, "m", _OpenAI_sanitizeLogValue).call(this, entry, seen)));
      };
    },
    set: (_facade, property, value) => globalThis.Reflect.set(logger4, property, value, logger4)
  });
...
// models.ts
var IMAGE_GEN_MODEL_PATTERN = /^(cogview|glm-image)/i;
var STATIC_MODELS = [
  {
    id: "glm-4.6",
    name: "GLM-4.6",
...
var STATIC_CHAT_MODEL_IDS = STATIC_MODELS.map((m) => m.id).filter((id) => !IMAGE_GEN_MODEL_PATTERN.test(id));

// provider.ts
var logger = createPluginLogger("qtap-plugin-z-ai");
var Z_AI_SUPPORTED_MIME_TYPES = [
  "image/jpeg",
  "image/png",
  "image/gif",
  "image/webp"
...
var ZAIProvider = class {
  ...
  async getAvailableModels(apiKey) {
    ...
    const merged = new Set(apiIds.filter((id) => !IMAGE_GEN_MODEL_PATTERN.test(id)));
...
// image-provider.ts
var logger2 = createPluginLogger("qtap-plugin-z-ai");
var ZAI_SENSITIVE_CONTENT_CODE = "1301";
function toZaiImageModerationError(error) {
  if (!error || typeof error !== "object") return error;
  const err = error;
  const rawCode = err.code ?? err.error?.code;
  const code = typeof rawCode === "number" || typeof rawCode === "string" ? String(rawCode) : void 0;
...
var SUPPORTED_MODELS = ["cogview-4-250304", "glm-image"];
var ZAIImageProvider = class {
  constructor() {
    this.provider = "Z_AI";
  ...
  async generateImage(params, apiKey) {
    ...
    try {
      ...
      if (mapped !== error) {
        logger2.info("Z.AI Images API refused the request as sensitive content", {
    ...
    if (!("data" in response) || !response.data || !Array.isArray(response.data)) {
      logger2.error("Invalid response from Z.AI Images API", {
        context: "ZAIImageProvider.generateImage"
      });
...
// index.ts
var logger3 = createPluginLogger("qtap-plugin-z-ai");
var Z_AI_IMAGE_CONSTRAINTS = {
  maxPromptBytes: 4e3,
...
var attachmentSupport = {
  ...
  notes: "Z.AI vision models accept image URLs or base64; limit 5MB per image, max 6000\xD76000 pixels.",
...
var plugin = {
  ...
  createImageProvider: (_baseUrl) => {
    return new ZAIImageProvider();
  },
  getAvailableModels: async (apiKey, _baseUrl) => {
    try {
  ...
  validateApiKey: async (apiKey, _baseUrl) => {
    try {
      const provider = new ZAIProvider();
  ...
  getImageProviderConstraints: () => Z_AI_IMAGE_CONSTRAINTS,
