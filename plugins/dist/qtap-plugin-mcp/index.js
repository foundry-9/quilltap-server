...
var z3rt = __toESM(require("zod/v3"), 1);
var z4mini = __toESM(require("zod/v4-mini"), 1);
...
var z4mini2 = __toESM(require("zod/v4-mini"), 1);
...
async function authInternal(provider, { serverUrl, authorizationCode, scope, resourceMetadataUrl, fetchFn }) {
  ...
  if (tokens?.refresh_token) {
    try {
      ...
      if (!(error instanceof OAuthError2) || error instanceof ServerError) {
      } else {
        throw error;
      }
    }
...
var MCPClient = class {
  ...
  async callTool(toolName, args) {
    ...
    const sdkContent = result.content || [];
    const content = sdkContent.map((block) => {
      if (block.type === "text") {
...
function convertToolsWithCollisionHandling(serverTools, existingToolNames = /* @__PURE__ */ new Set()) {
  const pendingTools = [];
  const nameUsage = /* @__PURE__ */ new Map();
  for (const [serverId, { displayName, tools: tools2 }] of serverTools) {
    for (const mcpTool of tools2) {
  ...
  for (const pending of pendingTools) {
    ...
    const quilltapName = hasCollision ? `${sanitizeToolName(pending.serverId)}_${pending.sanitizedName}` : pending.sanitizedName;
    const description = pending.mcpTool.description ? `[${pending.serverDisplayName}] ${pending.mcpTool.description}` : `[${pending.serverDisplayName}] Tool: ${pending.mcpTool.name}`;
    const parameters = {
      type: "object",
    ...
    mappings.push({
      quilltapName,
      mcpName: pending.mcpTool.name,
      // Original MCP name for calling the server
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
          if (typeof parsed.content === "string") {
            ...
            if (parsed.fuzzy_match) {
              metadata2.push(`fuzzy matched`);
            }
            if (parsed.actual_path && parsed.actual_path !== parsed.requested_path) {
              metadata2.push(`found at: ${parsed.actual_path}`);
            }
            if (metadata2.length > 0) {
              output = `[Note: ${metadata2.join(", ")}]
