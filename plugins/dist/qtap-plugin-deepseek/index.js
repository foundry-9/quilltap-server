        ...
        for await (const sse of messages) {
          ...
          if (sse.event === null || !sse.event.startsWith("thread.")) {
            ...
            try {
              ...
              logger2.error(`Could not parse message into JSON:`);
              logger2.error(`From chunk:`);
...
}, _OpenAI_sanitizeLogger = function _OpenAI_sanitizeLogger2(logger2) {
  if (__classPrivateFieldGet(_a3, _a3, "f", _OpenAI_sanitizedLoggers).has(logger2))
    return logger2;
  const sanitized = new globalThis.Proxy(/* @__PURE__ */ Object.create(null), {
    get: (_facade, property) => {
      const value = globalThis.Reflect.get(logger2, property, logger2);
      if (typeof value !== "function")
        return value;
      if (typeof property !== "string" || !["debug", "info", "warn", "error", "trace", "log"].includes(property)) {
        return value.bind(logger2);
      }
      return (...args) => {
        const seen = /* @__PURE__ */ new WeakMap();
        return globalThis.Reflect.apply(value, logger2, args.map((entry) => __classPrivateFieldGet(this, _OpenAI_instances, "m", _OpenAI_sanitizeLogValue).call(this, entry, seen)));
      };
    },
    set: (_facade, property, value) => globalThis.Reflect.set(logger2, property, value, logger2)
  });
...
var DeepSeekProvider = class extends OpenAICompatibleProvider {
  constructor(config2) {
    super({
      baseUrl: config2?.baseUrl ?? DEEPSEEK_BASE_URL,
      providerName: config2?.providerName ?? "DeepSeek",
  ...
  extractCacheUsage(usage) {
    ...
    if (hit === void 0 || hit <= 0) return void 0;
    ...
    try {
      ...
      return {
        ...
        usage: {
          ...
          // display. (DeepSeek's prompt_tokens = cache_hit + cache_miss.)
          promptTokens: Math.max(0, (response.usage?.prompt_tokens ?? 0) - (cacheUsage?.cacheReadInputTokens ?? 0)),
          completionTokens: response.usage?.completion_tokens ?? 0,
          totalTokens: Math.max(0, (response.usage?.total_tokens ?? 0) - (cacheUsage?.cacheReadInputTokens ?? 0))
        },
        raw: response,
    ...
    try {
      ...
      yield {
        ...
        usage: {
          ...
          promptTokens: Math.max(0, (usage?.prompt_tokens ?? 0) - (cacheUsage?.cacheReadInputTokens ?? 0)),
          completionTokens: usage?.completion_tokens ?? 0,
          totalTokens: Math.max(0, (usage?.total_tokens ?? 0) - (cacheUsage?.cacheReadInputTokens ?? 0))
        },
        toolCalls: toolCalls.length > 0 ? toolCalls : void 0,
        attachmentResults,
        rawResponse,
...
// index.ts
var logger = createPluginLogger("qtap-plugin-deepseek");
var metadata = {
