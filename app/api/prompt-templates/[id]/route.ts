...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    if (!session?.user?.id) {
      logger.warn('Unauthorized access attempt to GET /api/prompt-templates/[id]');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { id } = await context.params;
    logger.debug('Fetching prompt template', { templateId: id, userId: session.user.id });

    const repos = getRepositories();
    const template = await repos.promptTemplates.findById(id);

    if (!template) {
      logger.warn('Prompt template not found', { templateId: id, userId: session.user.id });
    ...
    logger.debug('Retrieved prompt template', { templateId: id, userId: session.user.id });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    const session = await getServerSession();
    if (!session?.user?.id) {
      logger.warn('Unauthorized access attempt to PUT /api/prompt-templates/[id]');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }
    ...
    logger.debug('Updating prompt template', { templateId: id, userId: session.user.id });

    const repos = getRepositories();
    const existingTemplate = await repos.promptTemplates.findById(id);

    if (!existingTemplate) {
      logger.warn('Prompt template not found for update', { templateId: id, userId: session.user.id });
      return NextResponse.json({ error: 'Template not found' }, { status: 404 });
    }
    ...
    if (!template) {
      logger.error('Failed to update prompt template', { templateId: id, userId: session.user.id });
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    const session = await getServerSession();
    if (!session?.user?.id) {
      logger.warn('Unauthorized access attempt to DELETE /api/prompt-templates/[id]');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    const { id } = await context.params;
    logger.debug('Deleting prompt template', { templateId: id, userId: session.user.id });

    const repos = getRepositories();
    const existingTemplate = await repos.promptTemplates.findById(id);

    if (!existingTemplate) {
      logger.warn('Prompt template not found for deletion', { templateId: id, userId: session.user.id });
      return NextResponse.json({ error: 'Template not found' }, { status: 404 });
    }
    ...
    if (!deleted) {
      logger.error('Failed to delete prompt template', { templateId: id, userId: session.user.id });
      return NextResponse.json({ error: 'Failed to delete template' }, { status: 500 });
    }

    logger.info('Prompt template deleted', { templateId: id, userId: session.user.id });
