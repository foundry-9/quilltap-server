...
CREATE TABLE "chats" (
  ...
  "transcriptVersion" INTEGER DEFAULT 0,      -- Monotonic transcript counter. Bumped by the message funnel (ChatMessagesOps.commitTranscriptChange, lib/database/repositories/chats-messages.ops.ts) on every add, add-batch, edit, delete and clear, in the same chat-row update, beside the publishRealtime('chats', id) hint an open Salon tab listens for. One bump per write, not per row. It is what makes the hinted transcript re-read conditional: the tab hands its last-seen value to GET /api/v1/messages?chatId=&action=transcript&knownVersion=N and the server answers {unchanged:true} without serializing the conversation. Exact under appends, edits AND deletions, which a since=<timestamp> cursor is not. Derived bookkeeping: NOT exported to .qtap (dropped by the NDJSON writer), and an import starts it at 0. Added by add-transcript-version-column-v1.
