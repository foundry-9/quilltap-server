...
**Status**: Bugs **1–126** are **fixed in v4**; nothing is open. **126** was
...
| 127 | [the progressions card prints raw `</code>, <code>` at the reader when two entries are unreadable](bugs/bug-127-progressions-invalid-ids-escaped.md) | 2026-09-08 | — | Low | The unparseable-entries line joins the ids with a literal `</code>, <code>` **inside a JSX expression**, so React escapes it: with two or more bad entries the reader is shown raw markup by the very sentence telling them their vault is damaged. One bad entry renders perfectly, which is why the suite never saw it | `components/characters/progressions/ProgressionsSection.tsx:132` | **Divergent, deliberately.** v5 renders the ids as separate `<code>` elements (`progressions-section.ts`); output is identical for one id. Pinned by two specs in `progressions-section.spec.ts`, but **no oracle compares this line**, so nothing trips automatically when v4 converges — retire the divergence by hand off this row |
| 128 | [the Salon's memory count is frozen at whatever it was when the tab opened, and a stale zero makes the Delete button a silent no-op](bugs/bug-128-stale-chat-memory-count.md) | 2026-09-08 | — | **Medium** (nothing errors and nothing is lost, but the sidebar states a falsehood about the user's data — `Delete Memories (0)` over a chat holding 59 — and the destructive control it labels early-returns on that falsehood, absorbing the click with no confirmation, no toast and no log line) | `fetchChatMemoryCount` (`useChatData.ts:105`) is read exactly once, by the mount-only initialization effect in `SalonView.tsx:801`; memories are written afterwards, turn after turn, by `MEMORY_EXTRACTION` and friends in the forked child, and nothing tells the client. There is no `memories` entry in `REALTIME_TOPICS`, no row in `queryKeysForTopic`, and no memory job type in `topicsForCompletedJob` — the server cannot say *this changed*, so the client never asks again. The tabbed workspace turns "once per mount" into "once per session": `WorkspaceHost.tsx:120` hides an inactive pane with a CSS class and keeps it mounted, which is what lets a streaming Salon survive a tab switch and also removes the accidental cure that leaving and returning used to be. Then `handleDeleteChatMemories` (`useMemoryActions.ts:18`) opens `if (chatMemoryCount === 0) return` — defensible against a true zero, indistinguishable from a broken click against a false one, and the button carries no `disabled`. Measured on Friday chat `27961b14`: 59 rows in the DB, `{"memoryCount":59}` from the live route, `(59)` on a fresh load of the same running build, `(0)` on the long-lived tab; the chat row was created 93 seconds before its first memory, so zero was true exactly once | A `memories` realtime topic — declared in `realtime.types.ts`, keyed by `queryKeys.memories.chatCount(chatId)`, mapped in `topic-map.ts`, and published from `topicsForCompletedJob` for the five memory job types (every one already carries `chatId` on its payload) plus `publishRealtime` at the two parent-side delete paths. The sidebar subscribes with `useRealtimeTopic('memories', fetchChatMemoryCount, id)`, which also refires on socket open, so a reconnect re-reads for free; no poll is added. `REPOSITORY_TOPICS` is deliberately **not** given a `memories` row — `firstIdArg` returns a positional id, so `memories.delete(memoryId)` would publish a hint every chat-scoped subscriber then filters out. Step 4 disarms the lie rather than the button: `disabled` at zero, and the count re-read immediately before the confirmation so the dialog can never quote a stale number. `chatPhotoCount` has the identical shape in the same hook and is fixed alongside, needing no new plumbing (`chats` is already published by the avatar and background jobs) | **Not yet assessed.** The port carries its own Salon sidebar; if it reads a count once at mount and gates a destructive action on it, it inherits this whole. Carry the invariant, not the code: a number rendered beside a destructive control must have a path by which the server can invalidate it |
