...
var FileTransport = class {
  ...
  async initializeDirectory() {
    try {
      ...
      const combinedLogPath = (0, import_path8.join)(this.logDir, "combined.log");
      const errorLogPath = (0, import_path8.join)(this.logDir, "error.log");
      try {
  ...
  async writeToFile(filename, content) {
    try {
      const filePath = (0, import_path8.join)(this.logDir, filename);
  ...
  async rotateFile(filename) {
    try {
      const basePath = (0, import_path8.join)(this.logDir, filename);
      const oldestPath = (0, import_path8.join)(this.logDir, `${filename}.${this.maxFiles}`);
      try {
        await import_fs.promises.unlink(oldestPath);
      } catch {
      }
      for (let i = this.maxFiles - 1; i >= 1; i--) {
        const oldPath = (0, import_path8.join)(this.logDir, `${filename}.${i}`);
        const newPath = (0, import_path8.join)(this.logDir, `${filename}.${i + 1}`);
        try {
          await import_fs.promises.rename(oldPath, newPath);
        } catch {
        }
      }
      const rotatedPath = (0, import_path8.join)(this.logDir, `${filename}.1`);
      try {
