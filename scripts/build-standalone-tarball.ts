...
const bootstrapJs = `// Auto-generated by scripts/build-standalone-tarball.ts. Do not edit.
// Mirrors the env setup from Next's generated standalone server.js so our own
// server-impl bundle boots cleanly:
//   - NODE_ENV=production keeps next() out of dev mode (which would try to
//     load router-utils/setup-dev-bundler — not in the standalone tarball).
//   - __NEXT_PRIVATE_STANDALONE_CONFIG, sourced from .next/required-server-files.json,
//     lets Next's loadWebpackHook silently tolerate the missing
//     next/dist/compiled/webpack files (not traced into the tarball).
'use strict';
