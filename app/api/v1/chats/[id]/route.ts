...
import type { ChatMetadata, ChatParticipantBase, MessageEvent, Character } from '@/lib/schemas/types';
...
export const POST = createAuthenticatedParamsHandler<{ id: string }>(async (req, { user, repos }, { id }) => {
  ...
  switch (action) {
    ...
    default:
      return badRequest(
        `Unknown action: ${action}. Available actions: regenerate-title, add-tag, remove-tag, impersonate, stop-impersonate, set-active-speaker, turn, add-participant, update-participant, remove-participant`
      );
  }
});
