...
- build: Switched notarization from Apple ID + app-specific password to App Store Connect API key — notarize.js now uses `APPLE_API_KEY_ID`, `APPLE_API_KEY_ISSUER`, and `APPLE_API_KEY_P8` (writes .p8 to temp file); added missing `appleApiKeyId` parameter required by @electron/notarize v3; release workflow updated with `CSC_NAME`, `CODESIGN_IDENTITY`, and API key secrets
