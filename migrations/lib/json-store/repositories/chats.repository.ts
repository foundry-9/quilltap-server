...
export class ChatsRepository extends BaseRepository<ChatMetadata> {
  ...
  async delete(id: string): Promise<boolean> {
    ...
    try {
      ...
      console.warn(`Failed to delete chat messages file for ${id}:`, { error: error instanceof Error ? error.message : String(error) });
    }
  ...
  async updateMessage(chatId: string, messageId: string, updates: Partial<ChatEvent>): Promise<ChatEvent | null> {
    try {
      ...
      console.error(`Failed to update message ${messageId} in chat ${chatId}:`, error instanceof Error ? error : new Error(String(error)));
      return null;
    }
  }
  ...
  async clearMessages(chatId: string): Promise<boolean> {
    try {
      ...
      console.error(`Failed to clear messages for chat ${chatId}:`, error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
  ...
  async migrateAllToParticipants(): Promise<{ migrated: number; total: number }> {
    try {
      ...
      console.error('Failed to migrate chats:', error instanceof Error ? error : new Error(String(error)));
