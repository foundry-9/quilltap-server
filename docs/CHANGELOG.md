...
- fix: Memories invisible after vector embedding BLOB migration — `MemoriesRepository` did not register `embedding` as a blob column, so after migration converted embeddings from JSON text to Float32 BLOBs, the raw `Buffer` values failed Zod schema validation and memories were silently filtered out; only memories with NULL embeddings (never embedded) survived; fix registers blob columns in `MemoriesRepository.getCollection()`, fixes `documentToRow` handling of empty embedding arrays, and fixes migration to NULL-ify empty TEXT embeddings instead of skipping them
