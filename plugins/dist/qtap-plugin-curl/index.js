...
function unicodeEscape(str2) {
  return str2.replace(/[\s\S]/g, (c2) => "\\u" + c2.charCodeAt().toString(16).padStart(4, "0"));
}
...
function charactersToCodes(str2) {
  return [...str2].map((c2) => "\\u" + c2.charCodeAt(0).toString(16).padStart(4, "0")).join("");
}
...
function makeReplacerFromDict(dict) {
  ...
  return (str2) => str2.replace(regex, replacer);
}
...
var formatRequestDetails = (details) => {
  ...
  if (details.headers) {
    details.headers = Object.fromEntries((details.headers instanceof Headers ? [...details.headers] : Object.entries(details.headers)).map(([name2, value]) => [
      name2,
      name2.toLowerCase() === "authorization" || name2.toLowerCase() === "api-key" || name2.toLowerCase() === "x-api-key" || name2.toLowerCase() === "x-amz-security-token" || name2.toLowerCase() === "cookie" || name2.toLowerCase() === "set-cookie" ? "***" : value
    ]));
  }
  ...
  if (!params || !hasAutoParseableInput(params)) {
    return {
      ...
      choices: completion.choices.map((choice2) => {
        assertToolCallsAreChatCompletionFunctionToolCalls(choice2.message.tool_calls);
        return {
          ...choice2,
          message: {
            ...choice2.message,
    ...
    for (const { delta, finish_reason, index, logprobs = null, ...other } of chunk.choices) {
      ...
      if (logprobs) {
        if (!choice2.logprobs) {
          ...
          if (content2) {
            (_a4 = choice2.logprobs).content ?? (_a4.content = []);
      ...
      if (function_call) {
        if (!choice2.message.function_call) {
          ...
          if (function_call.arguments) {
            (_c = choice2.message.function_call).arguments ?? (_c.arguments = "");
      ...
      if (content) {
        ...
        if (!choice2.message.refusal && __classPrivateFieldGet(this, _ChatCompletionStream_instances, "m", _ChatCompletionStream_getAutoParseableResponseFormat).call(this)) {
          choice2.message.parsed = choice2.message.content.trim() ? partialParse(choice2.message.content) : null;
        }
      }
      if (tool_calls) {
