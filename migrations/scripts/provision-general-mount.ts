/**
 * Migration: Provision the global Quilltap General mount point
 *
 * Companion to `provision-user-uploads-mount` and `provision-lantern-backgrounds-mount`.
 * Creates a single database-backed mount point named "Quilltap General"
 * (storeType=documents, mountType=database) and persists its id in
 * `instance_settings.generalMountPointId`. The mount houses curated, instance-wide
 * resources that aren't scoped to any project or character — currently a top-level
 * `Scenarios/` folder for general chat-starter scenarios offered in the New Chat
 * dialog regardless of project context.
 *
 * Idempotent: re-runs are a no-op when the setting is already populated and its
 * mount-point row still exists. If the setting points at a missing row (manual
 * deletion), the migration provisions a fresh mount and overwrites the setting.
 *
 * Migration ID: provision-general-mount-v1
 */
...
const MOUNT_NAME = 'Quilltap General';
const SETTINGS_KEY = 'generalMountPointId';
const SUBFOLDERS = ['Scenarios'] as const;
...
export const provisionGeneralMountMigration: Migration = {
  id: MIGRATION_ID,
  description:
    'Provision the global "Quilltap General" database-backed mount point and persist its id in instance_settings',
  ...
  async run(): Promise<MigrationResult> {
    ...
    try {
      ...
      const message = adopted
        ? `Adopted existing Quilltap General mount ${mountPointId}`
        : `Provisioned Quilltap General mount ${mountPointId}`;
      ...
      logger.error('Provision Quilltap General migration aborted', {
        context: `migration.${MIGRATION_ID}`,
        error: errorMessage,
      });
      return {
        ...
        success: false,
        itemsAffected: 0,
        message: 'Provision Quilltap General migration aborted',
        error: errorMessage,
        durationMs: Date.now() - startTime,
        timestamp: new Date().toISOString(),
      };
