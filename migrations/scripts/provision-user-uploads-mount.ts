/**
 * Migration: Provision the global Quilltap Uploads mount point
 *
 * Stage 1 of consolidating the catch-all `<filesDir>/_general/` namespace
 * onto the Scriptorium document-store pipeline (alongside Lantern Backgrounds
 * and per-character vaults). This migration creates a single database-backed
 * mount point named "Quilltap Uploads" (storeType=documents, mountType=database)
 * and persists its id in `instance_settings.userUploadsMountPointId` so the
 * runtime user-uploads bridge can find it without a name lookup.
 *
 * The mount is global (no `project_doc_mount_links` row). It surfaces in the
 * Scriptorium global mounts list and is the home for files produced outside
 * any project context:
 *
 *   - chat attachments in project-less chats (subfolder `chat/`)
 *   - paste/drag-drop image uploads outside a project (subfolder `images/`)
 *   - shell-tool "copy workspace → Files" with no project (subfolder `shell/`)
 *   - capabilities-report exports (subfolder `diagnostics/`)
 *   - backup-restore replay of project-less files (subfolder `restored/`)
 *
 ...
 * the setting.
 *
 * Migration ID: provision-user-uploads-mount-v1
 */
...
const MOUNT_NAME = 'Quilltap Uploads';
const SETTINGS_KEY = 'userUploadsMountPointId';
const SUBFOLDERS = ['chat', 'images', 'shell', 'diagnostics', 'restored', 'uploads'] as const;
...
export const provisionUserUploadsMountMigration: Migration = {
  id: MIGRATION_ID,
  description:
    'Provision the global "Quilltap Uploads" database-backed mount point and persist its id in instance_settings',
  ...
  async run(): Promise<MigrationResult> {
    ...
    try {
      ...
      const message = adopted
        ? `Adopted existing Quilltap Uploads mount ${mountPointId}`
        : `Provisioned Quilltap Uploads mount ${mountPointId}`;
      ...
      logger.error('Provision Quilltap Uploads migration aborted', {
        context: `migration.${MIGRATION_ID}`,
        error: errorMessage,
      });
      return {
        ...
        success: false,
        itemsAffected: 0,
        message: 'Provision Quilltap Uploads migration aborted',
        error: errorMessage,
        durationMs: Date.now() - startTime,
        timestamp: new Date().toISOString(),
      };
